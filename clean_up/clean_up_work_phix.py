"""
This script will remove the files generated by the phix filtering step of
preprocessing from nextflow's work directory, provided that we do not need
them anymore (i.e. we have the files in the output directory).
"""
...
class WorkDirCleaner():
    ...
    def log_to_sample(self, log_file):
        ...
        endings = ["_phix_filtered.bbduk.log"]
        for end in endings:
    ...
    def __call__(self):
        ...
        spades_process_logs = self.work_dir.glob("**/*phix_filtered.bbduk.log")
        ...
        for sample, process_dirs in sample_process_dirs.items():
            if not Path(self.output_dir,  sample, "preprocessed_reads").exists():
                logger.info(f"skipping {sample}")
                continue
            to_handle.extend(process_dirs)
        to_delete = []
        for process_dir in to_handle:
            to_delete.extend([file for file in process_dir.glob("*_phix_filtered_?.fastq.gz")
