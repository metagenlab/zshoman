...
workflow {
    ...
    samples = channel.fromList(samples_list)
        ...
        .map { id, r1, r2 ->
            ...
            def meta = [id: id, single_end: is_single, multi_lane: is_multi]
    ...
    qf_singletons = BBDUK_QUALITY_FILTERING.out.singletons.map { it -> [it[0] + [single_end: true], it[1]] }
    ...
    if (!params.skip_qc_preprocessed) {
        ...
        FASTQC_PREPROC(hf_reads)
        FASTQC_PREPROC_SINGLETONS(hf_singletons)
    }
    ...
    if (!params.skip_phanta) {
        ...
        PHANTA_PROFILE(
            hf_reads.filter { it -> (!params.resume_from_output) || (!file("${outdir_abs}/${it[0].id}/phanta").exists()) },
            params.phanta_db,
        )
    }
    ...
    if (!params.skip_assembly) {
        ...
        if (params.run_mag) {
            ...
            reads_and_contigs = preprocessed_samples.join(FILTER_SCAFFOLDS.out.prok_scaffolds)
        ...
        if (!params.skip_gene_catalog) {
            ...
            all_amino_acids = prokaryotic_genes_aa
                .mix(eukaryotic_genes_aa)
                .collectFile { row -> ["genes.faa.gz", row[1]] }
                .map { it -> [[id: 'all'], it] }
            all_nucleotides = prokaryotic_genes_nt
            ...
            bwa_mem_gc_ch = to_map.to_do.join(catalog_index).map { it -> [it[0], it[1], it[2], it[3]] }
        ...
        if (!params.skip_per_sample) {
            ...
            nt_tuples = nt_tuples.filter { it ->
                (!params.resume_from_output) || !file("${outdir_abs}/${it[0].id}/gene_counts").exists()
            }
