...
workflow {
    ...
    outdir_abs = java.nio.file.Paths.get(params.outdir).toAbsolutePath().toString()
    ...
    samples = channel.fromList(samples_list)
        .map { it ->
            [it[0].id, it[1], it[2]]
        }
        .groupTuple()
    ...
    lane_ch = samples.branch { it ->
        multi: it[0].multi_lane == true
        single: it[0].multi_lane == false
    }

    //Ensure a single list with r1 (reads[0]) followed by r2 (reads[1]) for each sample
    multi_ch = lane_ch.multi.map { meta, reads ->
        [meta, [reads[0], reads[1]].transpose().flatten()]
    }

    //Flatten reads list for single-lane samples
    single_ch = lane_ch.single.map { meta, reads ->
        [meta, reads.flatten()]
    }
    ...
    // folder in the ouput
    samples = cat_ch.branch { it ->
        already_preprocessed: params.resume_from_output && java.nio.file.Files.isDirectory(java.nio.file.Paths.get(outdir_abs, it[0].id, "preprocessed_reads"))
        to_preprocess: true
    }
    ...
    qf_singletons = BBDUK_QUALITY_FILTERING.out.singletons.map { it ->
        [it[0] + [single_end: true], it[1]]
    }
    ...
    hf_reads_split = hf_reads.branch { it ->
    ...
    paired_end_reads = hf_singletons.map { it -> [it[0] + [single_end: false], it[1]] }
    paired_end_reads = paired_end_reads.join(paired_end_reads_merged.merged.join(paired_end_reads_merged.unmerged))
    paired_end_reads = paired_end_reads.map { it -> [it[0], it[3] + [it[2]] + [it[1]]] }
    ...
    preprocessed_samples = preprocessed_samples.mix(
        samples.already_preprocessed.map { it ->
            it[0].single_end
                ? [
                    it[0],
                    [java.nio.file.Paths.get(outdir_abs, it[0].id, "preprocessed_reads", "${it[0].id}_host_filtered.fastq.gz")],
                ]
                : [
                    it[0],
                    [
                        java.nio.file.Paths.get(outdir_abs, it[0].id, "preprocessed_reads", "${it[0].id}_1_unmerged.fastq.gz"),
                        java.nio.file.Paths.get(outdir_abs, it[0].id, "preprocessed_reads", "${it[0].id}_2_unmerged.fastq.gz"),
                        java.nio.file.Paths.get(outdir_abs, it[0].id, "preprocessed_reads", "${it[0].id}_merged.fastq.gz"),
                        java.nio.file.Paths.get(outdir_abs, it[0].id, "preprocessed_reads", "${it[0].id}_host_filtered_singletons.fastq.gz"),
                    ],
                ]
        }
    )
        ...
        samples.already_preprocessed.map { it ->
            it[0].single_end
                ...
                : [
                    it[0],
                    [
                        java.nio.file.Paths.get(outdir_abs, it[0].id, "preprocessed_reads", "${it[0].id}_host_filtered_1.fastq.gz"),
                        java.nio.file.Paths.get(outdir_abs, it[0].id, "preprocessed_reads", "${it[0].id}_host_filtered_2.fastq.gz"),
                    ],
                ]
        }
    )
    ...
    if (!params.skip_motus) {
        ...
        samples_motus = preprocessed_samples.branch { it ->
            done: params.resume_from_output && java.nio.file.Files.isDirectory(java.nio.file.Paths.get(outdir_abs, it[0].id, "motus"))
            to_do: true
        }
        ...
        motus_profiles = motus_profiles.mix(
            samples_motus.done.map { it ->
                [
                    it[0],
                    java.nio.file.Paths.get(outdir_abs, it[0].id, "motus", "${it[0].id}.motus"),
                ]
            }
        )
    }
    ...
    if (!params.skip_phanta) {
        ...
        PHANTA_PROFILE(
            hf_reads.filter { it -> (!params.resume_from_output) || java.nio.file.Files.notExists(java.nio.file.Paths.get(outdir_abs, it[0].id, "phanta")) },
            params.phanta_db,
        )
    }
    ...
    if (!params.skip_assembly) {
        ...
        if (params.skip_gene_catalog) {
            preprocessed_samples = preprocessed_samples.filter { it ->
                (!params.resume_from_output) || java.nio.file.Files.notExists(java.nio.file.Paths.get(outdir_abs, it[0].id, "annotations")) || java.nio.file.Files.notExists(java.nio.file.Paths.get(outdir_abs, it[0].id, "gene_counts"))
            }
        }

        // Skip samples for which spades has already been run and readd afterwards
        samples_spades = preprocessed_samples.branch { it ->
            done: params.resume_from_output && java.nio.file.Files.isDirectory(java.nio.file.Paths.get(outdir_abs, it[0].id, "assembly"))
            to_do: true
        }

        SPADES(samples_spades.to_do.map { it -> [it[0], it[1], [], []] }, [], [])

        scaffolds = SPADES.out.scaffolds.mix(
            samples_spades.done.map { it ->
                [
                    it[0],
                    java.nio.file.Paths.get(outdir_abs, it[0].id, "assembly", "${it[0].id}.scaffolds.fa.gz"),
                ]
            }
        )
        graphs = SPADES.out.gfa.mix(
            samples_spades.done.map { it ->
                [
                    it[0],
                    java.nio.file.Paths.get(outdir_abs, it[0].id, "assembly", "${it[0].id}.assembly.gfa.gz"),
                ]
            }
        )
        paths = SPADES.out.assembly_paths.mix(
            samples_spades.done.map { it ->
                [
                    it[0],
                    java.nio.file.Paths.get(outdir_abs, it[0].id, "assembly", "${it[0].id}.scaffolds.paths.gz"),
                ]
            }
        )
        ...
        ASSEMBLY_STATS(filtered_assembly)
        ...
        eukaryotic_genes = METAEUK_EASYPREDICT(
            FILTER_SCAFFOLDS.out.euk_scaffolds.filter { it -> it[1].size() > 0 },
            params.metaeuk_db,
        )
        ...
        if (!params.skip_gene_catalog) {
            ...
            all_amino_acids = prokaryotic_genes.amino_acid_fasta
                .mix(eukaryotic_genes_aa)
                .collectFile { row -> ["genes.faa.gz", row[1]] }
                .map { it -> [[id: 'all'], it] }
            all_nucleotides = prokaryotic_genes.nucleotide_fasta
                .mix(eukaryotic_genes_nt)
            ...
            gene_catalog_aa = SEQTK_SUBSEQ(all_amino_acids, headers.map { it -> it[1] }.first()).sequences

            catalog_index = BWA_INDEX_GC(gene_catalog_nt).index
            aligned_reads = BWA_MEM_GC(reads.combine(catalog_index).map { it -> [it[0], it[1], it[3]] }, false).bam
            ...
            EGGNOGMAPPER_GC(gene_catalog_aa, params.eggnog_db, params.eggnog_dbdir, [[:], params.eggnog_dmnd])
        }
        ...
        if (!params.skip_per_sample) {
            ...
            nt_tuples = nt_tuples.filter { it ->
                (!params.resume_from_output) || java.nio.file.Files.notExists(java.nio.file.Paths.get(outdir_abs, it[0].id, "gene_counts"))
            }
            ...
            aa_tuples = aa_tuples.filter { it ->
                (!params.resume_from_output) || java.nio.file.Files.notExists(java.nio.file.Paths.get(outdir_abs, it[0].id, "annotations"))
            }
            amino_acids = CAT_AA(aa_tuples).file_out
            EGGNOGMAPPER_SAMPLES(amino_acids, params.eggnog_db, params.eggnog_dbdir, [[:], params.eggnog_dmnd])
        }
    }
    workflow.onComplete {
        println("Done!!!!!!!!!!!!")
